...
(defn outbound-components
    ...
    (->> (.getTargets worker-context component-id)
         clojurify-structure
         (map
             (fn [[stream-id component->grouping]]
                 [stream-id
                  (outbound-groupings
                   worker-context
                   component-id
...
(defn throttled-report-error-fn [executor]
    (let [storm-conf          (:storm-conf executor)
        ...
        (fn [error]
            (log-error error)
            (when
                (> (time-delta @interval-start-time)
                   error-interval-secs)
...
(defn mk-executor-data [worker executor-id]
    (let [worker-context         (worker-context worker)
        ...
        (recursive-map
         ...
         :storm-cluster-state
                                          (cluster/mk-storm-cluster-state (:cluster-state worker)
         ...
         :spout-throttling-metrics
                                          (if (= executor-type :spout)
...
(defmethod mk-threads :spout
    [executor-data task-datas initial-credentials]
        ...
        [(async-loop
          (fn []
              ...
              (doseq [[task-id task-data] task-datas
                      :let                [^ISpout spout-obj
                                           (:object task-data)
                                           tasks-fn
                                           (:tasks-fn task-data)
                                           send-spout-msg
                                           (fn [out-stream-id values message-id out-task-id]
                  ...
                  (.open spout-obj
                         ...
                         (SpoutOutputCollector.
                             (reify
                                 ...
                                 (^void emitDirect
                                     [this ^int out-task-id ^String stream-id
                                      ^List tuple
                                      ^Object message-id]
...
(defmethod close-component :spout
    [executor-data spout]
    (.close spout))

(defmethod close-component :bolt
    [executor-data bolt]
    (.cleanup bolt))

;; TODO: refactor this to be part of an executor-specific map
(defmethod mk-executor-stats :spout
    [_ rate]
    (stats/mk-spout-stats rate))

(defmethod mk-executor-stats :bolt
    [_ rate]
